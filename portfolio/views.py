...
def portfolio(request, pk):
    ...
    sum_of_current_stocks_value = 0
    sum_of_initial_stocks_value = 0

    # Loop through each stock and add the value to the total
    for stock in stocks:
        sum_of_current_stocks_value += stock.current_stock_value()
        sum_of_initial_stocks_value += stock.initial_stock_value()

    portfolio_initial_investments = float(sum_of_initial_stocks_value) + \
                                    float(sum_acquired_value['acquired_value__sum']) + \
                                    float(sum_acquired_value_mf['acquired_value__sum'])
    portfolio_current_investments = float(sum_of_current_stocks_value) + \
    ...
    return render(request, 'portfolio/portfolio.html', {'customers': customers, 'investments': investments,
                                                        ...
                                                        'sum_of_current_stocks_value': float(sum_of_current_stocks_value),
                                                        'sum_of_initial_stocks_value': float(sum_of_initial_stocks_value),
                                                        'portfolio_initial_investments': portfolio_initial_investments,
                                                        'portfolio_current_investments': portfolio_current_investments,
                                                        })


# Rest API to Lists all customers
...
def customer_pdf(request, pk):
    ...
    html = render_to_string('portfolio/portfolio.html',{'customers': customers, 'investments': investments,
                                                        'stocks': stocks, 'mutualfunds': mutualfunds,
                                                        'sum_acquired_value': sum_acquired_value['acquired_value__sum'],
                                                        'sum_recent_value': sum_recent_value['recent_value__sum'],
                                                        'sum_acquired_value_mf': sum_acquired_value_mf['acquired_value__sum'],
    ...
    response['Content-Disposition'] = 'filename=\
        "portfolio_{}.pdf"'.format(customer.pk)
